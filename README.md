...
| **usage_log_path** | Local JSONL file used for `/monitor` usage history and cost windows.<br /><br />Suggested default: `usage.jsonl` |
| **cost_estimation** | Configures `/monitor` cost estimates with fallback token-per-character ratios and per-model `input_per_million` / `output_per_million` prices. |
